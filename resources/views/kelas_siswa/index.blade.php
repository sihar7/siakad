...
@section('content')
   ...
   @endphp --}}
                    ...
                    @foreach ($classes as $cs)
                        @php
                            $students = \App\ClassStudent::where('class_room_id', $cs->id)
                                ->where('semester_id', $_GET['semester'])
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <a href="" class="btn btn-primary btn-sm mb-3 tambah" data-toggle="modal"
                                        data-target="#exampleModal" data-id="{{ $cs->id }}"
                                        data-class="Kelas {{ $cs->nama }}">Tambah</a>
                            ...
                            <table class="table" id="modalTable">
                                ...
                                <tbody>
                                    @foreach ($studentNotExists as $sne)
                                        @php
                                            $last = \App\ClassStudent::where('student_id', $sne->id)
                                                ->latest()
                                                ->first();
                                            // dd($last);
                                        @endphp
                                            ...
                                            <td>
                                                <div class="custom-control custom-checkbox">
                                                    <input type="checkbox" class="custom-control-input"
                                                        id="student_id{{ $sne->id }}" name="student_id[]"
                                                        value="{{ $sne->id }}">
                                                    <label class="custom-control-label"
                                                        for="student_id{{ $sne->id }}">{{ $sne->nama }}</label>
                                                </div>
                                            </td>
                                            {{-- @foreach ($last as $l) --}}
